...
<body>
    <div class="card">
        ...
        <form id="genForm">
            ...
            <div class="section-label">4. Inference Flags</div>
            <div style="display:flex; gap:20px; align-items:center; margin: 8px 0;">
                <label style="display:flex; align-items:center; gap:6px; cursor:pointer;">
                    <input type="checkbox" name="use_ema" value="true" style="width:auto; margin:0;">
                    EMA weights
                </label>
                <label style="display:flex; align-items:center; gap:6px; cursor:pointer;">
...
def run_inference(job_id, input_path, control_type, guide_path, bbox_vals, mc_res, use_ema, flashvdm):
    jobs[job_id]["status"] = "processing"
    search_start_time = time.time() - 30 
    
    # 1. Conditioning Logic - Now injecting mc_res into the JSON payload
    ...
    try:
        # 2. RUN SUBPROCESS
        cmd = [VENV_PYTHON, "inference.py", "--control_type", control_type]
        if use_ema:
            cmd.append("--use_ema")
        if flashvdm:
            cmd.append("--flashvdm")
        ...
        if new_files:
            ...
            jobs[job_id]["status"] = "failed"
            print(f"ERROR: No new mesh found among {len(all_glbs)} files.")
