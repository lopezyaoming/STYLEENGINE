...
def run_inference(job_id, input_path, control_type, guide_path, bbox_vals, mc_res):
    ...
    try:
        # 2. RUN SUBPROCESS (Removed unsupported --mc_res flag)
        cmd = [
...
async def generate(
    ...
    background_tasks.add_task(run_inference, job_id, input_path, control_type, guide_path, bbox_vals, int(mc_res))
