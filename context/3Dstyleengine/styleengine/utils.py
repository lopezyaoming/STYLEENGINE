...
def get_prompt_from_text_editor():
    """
    Read the prompt from the currently active text editor.
    Falls back to STYLEENGINE_Prompt if no text editor is active.
    This allows users to switch between templates without copying content.
    
    Returns:
        str: The cleaned prompt text, or empty string if not found
    """
    # Try to get the currently active text from any open text editor
    active_text = None
    source_name = None
    ...
    if not active_text:
        ...
        if not text:
            print("[Style Engine] ⚠️ No text editor active and STYLEENGINE_Prompt not found!")
            return ""
        
        active_text = text
    
    # Get raw content from active or fallback text
    raw_content = active_text.as_string()
    print(f"[Style Engine] 📖 Reading from: {source_name}")
    print(f"[Style Engine] 📖 Content ({len(raw_content)} chars):")
    print(f"[Style Engine]    '{raw_content[:100]}...'" if len(raw_content) > 100 else f"[Style Engine]    '{raw_content}'")
